...
function inspectWorkbook(buffer, sheetName) {
  ...
  for (const match of xml.matchAll(/<(?:[\w-]+:)?row\b([^>]*)>([\s\S]*?)<\/(?:[\w-]+:)?row>/g)) {
    ...
    if (!headers && Object.values(values).includes("姓名") && Object.values(values).includes("课后作业错题") && Object.values(values).includes("课后作业评价")) {
      headers = Object.fromEntries(Object.entries(values).map(([column, value]) => [value, column]));
      headerRow = row;
    } else if (headers && row > headerRow && values[headers["姓名"]]) {
      rows.push({ row, name: values[headers["姓名"]], mistakes: values[headers["课后作业错题"]] ?? "", feedback: values[headers["课后作业评价"]] ?? "" });
    }
  }
  if (!headers) fail("Required headers are missing.");
  return { zip, path, xml, headers, rows };
}
...
if (!mode || !inputPath || !sheetName) fail("Usage: patch_xlsx.mjs <inspect|write> <file.xlsx> <数学|物理> [base64-json-updates]");
const before = await fs.readFile(inputPath);
const inspected = inspectWorkbook(before, sheetName);
if (mode === "inspect") {
  console.log(JSON.stringify({ sheet: sheetName, feedbackColumn: inspected.headers["课后作业评价"], rows: inspected.rows }));
} else if (mode === "write") {
  ...
  const changedXml = patchSheet(inspected.xml, inspected.headers["课后作业评价"], updates);
  ...
  console.log(JSON.stringify({ sheet: sheetName, writtenRows: updates.map((item) => Number(item.row)), feedbackColumn: inspected.headers["课后作业评价"] }));
